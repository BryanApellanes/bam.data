...
[IntegrationTestMenu("Firebird Collection should", "fbc")]
public class FirebirdCollectionShould : IntegrationTestMenuContainer
{
    private const string ContainerName = "bam-data-test-firebird";
    private const string Password = "BamTest1!";

    private static FirebirdSqlDatabase SetupDb()
    {
        FbConnection.ClearAllPools();

        FirebirdSqlDatabase db = new FirebirdSqlDatabase("localhost", "/firebird/data/bamtest.fdb",
            new FirebirdSqlCredentials { UserId = "SYSDBA", Password = Password });

        PodmanContainerHelper.WaitForReady(ContainerName, 60, () =>
        {
            using var conn = new FbConnection(db.ConnectionString);
            conn.Open();
            return true;
        });
    ...
    {
        FirebirdSqlDatabase db = SetupDb();
        ...
        .When<IDatabase>("saves order with 2 lines and reloads", (database) =>
        {
            ...
            return TestOrder.Where(c => c.CustomerName == "Fb-Alice", database);
        })
        .TheTest
        .ShouldPass(because =>
        {
    ...
    public void XrefCollectionSaveAndReload()
    {
        ...
        {
            TestOrder order = new TestOrder(database) { CustomerName = "Fb-Bob" };
            order.Save(database);
            ...
            return TestOrder.Where(c => c.CustomerName == "Fb-Bob", database);
        })
        .TheTest
        .ShouldPass(because =>
        {
    ...
    public void XrefRemoveAndClear()
    {
        ...
        {
            TestOrder order = new TestOrder(database) { CustomerName = "Fb-Charlie" };
            order.Save(database);
            ...
            xref.Remove(tag1);
            var afterRemoveCol = TestOrder.Where(c => c.CustomerName == "Fb-Charlie", database);
            int countAfterRemove = afterRemoveCol[0].TestTags.Count;

            afterRemoveCol[0].TestTags.Clear(database);
            var afterClearCol = TestOrder.Where(c => c.CustomerName == "Fb-Charlie", database);
