...
namespace Bam.Container.Docker
{
    public class DockerRuntime : IContainerRuntime
    {
        ...
        {
            System.Console.WriteLine($"[docker] docker {args}");
            ...
            {
                System.Console.WriteLine($"[docker]   stdout: {result.StandardOutput.Trim()}");
            }

            if (!string.IsNullOrWhiteSpace(result.StandardError))
            {
                System.Console.WriteLine($"[docker]   stderr: {result.StandardError.Trim()}");
            }

            if (result.ExitCode != 0 && !string.IsNullOrWhiteSpace(result.StandardError))
            {
                throw new InvalidOperationException($"docker {args} failed: {result.StandardError}");
            }
